...
task UpdateManifest -If {$BuildMode -eq "Release"} {
    ...
    if ($ReleaseVersion -notlike "vNext*") {
        $ManifestPath += @{ ModuleVersion = $ReleaseVersion }
    }

    Update-ModuleManifest @ManifestData

    if ($ReleaseVersion -like "vNext*") {
...
task GenerateMarkdownHelp -If {($BuildMode -eq "Snapshot") -or ($BuildMode -eq "Release")} {
    $ModulePath = "$PSScriptRoot\src\$ModuleName.psm1"
...
task MarkdownHelpToHtml -If {($BuildMode -eq "Snapshot") -or ($BuildMode -eq "Release")} {
    ...
    Convert-MarkdownToHTML -Path "docs" -Destination "out\docs" -Template "src\MarkdownToHtmlTemplate" | Out-Null
...
task Zip -If {($BuildMode -eq "Snapshot") -or ($BuildMode -eq "Release")} {
    ...
    if ($ReleaseVersion) {
        Compress-Archive -Path "out\tozip\*" -DestinationPath "out\$ModuleName-v$ReleaseVersion.zip"
    } else {
        Compress-Archive -Path "out\tozip\*" -DestinationPath "out\$ModuleName-snapshot$( Get-Date -Format yyMMdd ).zip"
    }

    Remove-Item -Path "out\tozip" -Recurse -Force
}